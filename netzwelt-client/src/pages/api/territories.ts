...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      let getTerritories = await axios.get(
      ...
      if (getTerritories.status == 200) {
        if (getTerritories.data) {
          ...
          let cleanTerritories = territories
            .filter((item: any) => item.parent == null)
            .sort((a: any, b: any) => a.name.localeCompare(b.name));

          cleanTerritories.map((item: any) => {
            let children = territories
              .filter((childItem: any) => childItem.parent == item.id)
              .sort((a: any, b: any) => a.name.localeCompare(b.name));
          ...
          cleanTerritories.map((item: any) => {
            item.children.map((subItem: any) => {
              let children = territories
                .filter((childItem: any) => childItem.parent == subItem.id)
                .sort((a: any, b: any) => a.name.localeCompare(b.name));
          ...
          cleanTerritories.map((item: any) => {
            item.children.map((subItem: any) => {
              subItem.children.map((finalSubItem: any) => {                
                let finalChildren = territories
                  .filter(
                    (childItem: any) => childItem.parent == finalSubItem.id
                  )
                  .sort((a: any, b: any) => a.name.localeCompare(b.name));
                finalSubItem.children = finalChildren;
              });
            });
          });
